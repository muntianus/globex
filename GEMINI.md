# Globex B2B Marketplace - Руководство для Gemini

Этот документ предоставляет всю необходимую информацию для управления, запуска и разработки проекта Globex.

## 📂 Обзор структуры проекта

Проект представляет собой монорепозиторий, содержащий три основных компонента:

- **`/frontend`**: Клиентское веб-приложение, разработанное на React и TypeScript.
- **`/backend`**: Серверное приложение (API) на Python с использованием фреймворка FastAPI.
- **`/b2b_marketplace_app`**: Кроссплатформенное мобильное приложение на Flutter.
- **`/docker-compose.yml`**: Основной файл для оркестрации всех сервисов в среде разработки.
- **`/ssl`**: Директория для хранения SSL-сертификатов для `nginx`.

## 🚀 Быстрый старт: Запуск полной среды

Вся среда разработки (frontend, backend, база данных) управляется через Docker Compose.

**Команда для запуска:**
```bash
docker-compose up --build
```
Эта команда соберет образы контейнеров (если они изменились) и запустит все сервисы.

**Доступные сервисы:**
- **Веб-приложение**: [http://localhost:3000](http://localhost:3000) (React dev server)
- **Backend API**: [http://localhost:8000](http://localhost:8000) (документация API доступна по [http://localhost:8000/docs](http://localhost:8000/docs))
- **Прокси (Nginx)**: [http://localhost](http://localhost) и [https://localhost](https://localhost)
...
- **Запуск тестов**:
  ...
  cd backend && pytest
  ```
  *(Примечание: `pytest` необходимо установить отдельно: `pip install pytest`)*

### Мобильное приложение (`/b2b_marketplace_app`)

- **Установка зависимостей**:
  ```bash
  cd b2b_marketplace_app && flutter pub get
  ```
- **Запуск приложения**:
  ```bash
  cd b2b_marketplace_app && flutter run
  ```
- **Запуск тестов**:
  ```bash
  cd b2b_marketplace_app && flutter test
  ```

## ✅ Качество кода

- **Frontend**: Используется ESLint, конфигурация находится в `package.json`.
- **Backend**: Код следует стандартам FastAPI.
- **Мобильное приложение**: Используется `very_good_analysis` для статического анализа.
